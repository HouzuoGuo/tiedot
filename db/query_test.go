...
func TestQuery(t *testing.T) {
	...
	defer col.Close()
	docs := []string{
		`{"a": {"b": [1]}, "c": 1, "d": 1, "f": 1, "g": 1, "special": {"thing": null}, ` + PaddingAttr(chunkfile.DOC_MAX_ROOM/3) + `}`,
		`{"a": {"b": 1}, "c": [1], "d": 2, "f": 2, "g": 2, ` + PaddingAttr(chunkfile.DOC_MAX_ROOM/3) + `}`,
		`{"a": [{"b": [2]}], "c": 2, "d": 1, "f": 3, "g": 3, ` + PaddingAttr(chunkfile.DOC_MAX_ROOM/3) + `}`,
		`{"a": {"b": 3}, "c": [3], "d": 2, "f": 4, "g": 4, ` + PaddingAttr(chunkfile.DOC_MAX_ROOM/3) + `}`,
		`{"a": {"b": [4]}, "c": 4, "d": 1, "f": 5, "g": 5, ` + PaddingAttr(chunkfile.DOC_MAX_ROOM/3) + `}`,
		`{"a": [{"b": 5}, {"b": 6}], "c": 4, "d": 1, "f": 5, "g": 5, ` + PaddingAttr(chunkfile.DOC_MAX_ROOM/3) + `}`}
	ids := [6]uint64{}
	...
	q, err = runQuery(`{"has": ["a", "b"], "limit": 3}`, col)
	if err != nil {
		t.Fatal(err)
	}
	...
	// int collection scan
	q, err = runQuery(`{"int-from": 2, "int-to": 4, "in": ["g"]}`, col)
	if err != nil {
		t.Fatal(err)
	}
	if !ensureMapHasKeys(q, ids[1], ids[2], ids[3]) {
	...
	q, err = runQuery(`{"int-from": 2, "int-to": 4, "in": ["g"], "limit": 2}`, col)
	if err != nil {
		t.Fatal(err)
	}
	if !ensureMapHasKeys(q, ids[1], ids[2]) {
	...
	q, err = runQuery(`{"re": "^[2345]$", "in": ["f"], "limit": 3}`, col)
	if err != nil {
		t.Fatal(err)
	}
	if !ensureMapHasKeys(q, ids[1], ids[2], ids[3]) {
